<?php
 ...
 $GLOBALS['connection_oop'] = new mysqli('localhost', 'root', '', 'tdws');
 $GLOBALS['connection_pro'] = mysqli_connect("localhost","root","","tdws");
...
function kvToUpdateString($array){
  // UPDATE `bookings` SET `BookingNo` = '456ee', `POL` = '3213ccc' WHERE `bookings`.`ID` = 2
  $data= '';
	$i=count($array);
	foreach($array as $key => $value){
		if($i==1){
